...
//Datos de WiFi
const char* ssid = "Totalplay-D9A0_2.4Gnormal";  // Aquí debes poner el nombre de tu red
const char* password = "Soporte2022";  // Aquí debes poner la contraseña de tu red

//Datos del broker MQTT son los datos de mi IP, porque es un broker local (localhost) 
const char* mqtt_server = "192.168.100.130"; // Si estas en una red local, coloca la IP asignada, en caso contrario, coloca la IP publica
IPAddress server(192,168,100,130);
...
void loop() {
  ...
  if (stringComplete) {
    ...
    client.publish("codigoIoT/ejemplo/mqtt/mq", charBuf);
    // clear the string:
    inputString = "";
...
//read the incoming string:
  ...
  String mqs = Serial2.readString();
  String MQS = "{"+String(mqs)+"}";
//    
}
//   
else{
    String json = "{\"id\":\"DHT11\",\"temp\":"+String(t)+",\"hum\":"+String(h)+"}";
    Serial.println(json); // Se imprime en monitor solo para poder visualizar que el string esta correctamente creado
